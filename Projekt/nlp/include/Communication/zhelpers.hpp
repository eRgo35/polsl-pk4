...
inline static std::string s_recv(zmq::socket_t &socket, int flags = 0) {
  ...
  auto recv_flags =
      (flags == 0) ? zmq::recv_flags::none : zmq::recv_flags::dontwait;
...
inline static bool s_send(zmq::socket_t &socket, const std::string &string,
  ...
  bool rc =
      socket.send(message, static_cast<zmq::send_flags>(flags)).has_value();
  return (rc);
}
...
inline static bool s_sendmore(zmq::socket_t &socket,
  ...
  bool rc = socket.send(message, static_cast<zmq::send_flags>(ZMQ_SNDMORE))
                .has_value();
  return (rc);
}
