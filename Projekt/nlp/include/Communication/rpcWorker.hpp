...
class mdwrk {
 public:
  ...
  mdwrk(std::string broker, std::string service, int verbose)
      : m_broker(broker), m_service(service), m_verbose(verbose) {
  ...
  void connect_to_broker() {
    ...
    if (m_verbose)
      console->log("I: connecting to broker at ", m_broker.c_str(), "...");
  ...
  zmsg *recv(zmsg *&reply_p) {
    ...
    while (!s_interrupted) {
      ...
      if (items[0].revents & ZMQ_POLLIN) {
        ...
        if (m_verbose) {
          console->warn("W: disconnected from broker - retrying...");
        }
        s_sleep(m_reconnect);
        connect_to_broker();
      }
    ...
    if (s_interrupted)
      console->error("W: interrupt received, killing worker...");
    return NULL;
  }
